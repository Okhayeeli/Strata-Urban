...
//        booking.setClientId(); //TODO AFTER AUTHENTICATION
    ...
    public void rateService(RateRequestBodyDTO rateRequestBodyDTO) {
        Supplier existingSupplier = supplierRepository.findById(rateRequestBodyDTO.getId())
                .orElseThrow(()-> new RuntimeException("Supplier wasn't found"));

        existingSupplier.setRating(findRatingAverage(rateRequestBodyDTO.getRate(),
                existingSupplier.getRating(), existingSupplier.getNumberOfRatings()));

        existingSupplier.setNumberOfRatings(existingSupplier.getNumberOfRatings()+1);
